...
% LMI3 = cell(4,4);
% LMI4 = cell(4,4);
LMI2 = [0.5*W   W*E2'           W*Nz';
        E2*W    Lambda2*eye(1)  zeros(1,3);
        Nz*W    zeros(3,1)      Q-Lambda2*(H2*H2')];
LMI3 = [0.5*Q       (sqrt(Q1)*Q)'   (sqrt(R)*Y)'    Q           (E1*Q+E3*Y)'    (Az*Q+Bz*Y)';
        sqrt(Q1)*Q  2*gamma*eye(3)  zeros(3,1)      zeros(3,3)  zeros(3,1)      zeros(3,3);
        sqrt(R)*Y   zeros(1,3)      2*gamma*eye(1)  zeros(1,3)  zeros(1,1)      zeros(1,3);
        Q           zeros(3,3)      zeros(3,1)      2*W         zeros(3,1)      zeros(3,3);
        E1*Q+E3*Y   zeros(1,3)      zeros(1,1)      zeros(1,3)  Lambda1*eye(1)  zeros(1,3);
        Az*Q+Bz*Y   zeros(3,3)      zeros(3,1)      zeros(3,3)  zeros(3,1)      Q-Lambda1*(H1*H1')];
LMI4 = [del_umax^2*eye(1)   Y;
        Y'              Q];    
LMI5 = [Q           H1*Lambda1;
        Lambda1*H1' Lambda1*eye(1)];
LMI6 = [Q           H2*Lambda2;
        Lambda2*H2' Lambda2*eye(1)];
