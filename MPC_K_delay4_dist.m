...
R = 0.1;
rou = 3;
beta = 0.9;   % fault related coeffi.
nx = 2;     % Number of states
nu = 1;     % Number of inputs
delay_bound = 4;
T = 109+delay_bound;
ommega = -0.1+0.2*rand(T+1,1);  % disturbance
% ommega = sqrt(0.1)*randn(T+1,1);
delta_u = cell(1,T);
delta_x = cell(2,T+1);
delta_y = cell(1,T+1);
y = cell(1,T+1);
...
% z{1} = [delta_x{1}; e{1}];
% z_rec = [z{1}];
Qvj = sdpvar(3,3,'symmetric');
% Qlj = sdpvar(3,3,'symmetric');
Y = sdpvar(1,3);
G = sdpvar(3,3,'full');
...
for i = 1:1:delay_bound+1
    delta_ommega{i} = ommega(i+1) - ommega(i);
    delta_x{i} = [0.1;0.1];
    delta_u{i} = 0.5;
    delta_y{i} = 0.5;
    y{i} = 0;
...
LMI2 = [Omega11     zeros(3,3)   zeros(3,2)           G'          Omega51'    (sqrt(Q)*G)'    (sqrt(R)*Y)';
        ...
        Omega51     Nz*W         Gp                   zeros(3,3)  Qvj         zeros(3,3)       zeros(3,1) ; 
...
for k = delay_bound+1:1:T
    delta_ommega{k} = ommega(k+1) - ommega(k);
...
%         e{k+1} = y{k}-c{k+1};
    ...
    LMI1 = [1       z{k}'       z{k-1}'     z{k-2}'     z{k-3}'     z{k-4}';
            z{k}    Qvj         zeros(3,3)  zeros(3,3)  zeros(3,3)  zeros(3,3);
            z{k-1}  zeros(3,3)  W           zeros(3,3)  zeros(3,3)  zeros(3,3);
            z{k-2}  zeros(3,3)  zeros(3,3)  W           zeros(3,3)  zeros(3,3);
            z{k-3}  zeros(3,3)  zeros(3,3)  zeros(3,3)  W           zeros(3,3);
            z{k-4}  zeros(3,3)  zeros(3,3)  zeros(3,3)  zeros(3,3)  W];   %/tao = 3 
    ...
    u{k+1} = u{k} + delta_u{k}; 
%     x{k+1} = x{k} + delta_x{k};
    z{k+1} = Az*z{k} + Bz*beta*delta_u{k} + Nz*z{k-d}+Gp*E*cell2mat(delta_ommega(k));
    x{k+1} = z{k}(1:2) + x{k};
    delta_y{k+1} = Cz*z{k+1};
    y{k+1} = y{k} + delta_y{k+1};
    z_rec = [z_rec z{k+1}];
    delu_rec = [delu_rec value(delta_u{k})];
    z_rec = [z_rec value(z{k})];
    dely_rec = [dely_rec value(delta_y{k+1})];
    x_rec = [x_rec x{k+1} ];
    u_rec = [u_rec u{k+1}];
    y_rec = [y_rec y{k+1}];
    e_rec = [e_rec abs(z{k+1}(3))];
end
%%
if 0
figure(1)
...
if 1
figure
plot(x_rec(1,:),'b-.','linewidth',1);
% plot(z_rec(1,:),'linewidth',1);
hold on
plot(x_rec(2,:),'b-.','linewidth',1);
...
hold on 
plot(y_rec,'b-.','linewidth',1);
xlabel('step')
ylabel('Output-Y')
legend('setpoint','statefeedback');

figure
plot(e_rec,'b-.','linewidth',1);
xlabel('t/sampling')
ylabel('e(k)')
end
