...
T1 = 49+delay_bound;
T2 = T1+49;
% ommega = -(1+2.*rand(T+1,1))*0.1;  % disturbance
ommega = sqrt(0.1)*randn(T2+1,1);
delta_u = cell(1,T2);
delta_x = cell(2,T2+1);
delta_y = cell(1,T2+1);
y = cell(1,T2+1);
e = cell(1,T2+1);
c = cell(1,T2+1);   %setpoint
x = cell(1,T2+1);
u = cell(1,T2+1);
delta_ommega = cell(1,T2+1);
x{1} = [1;1];
u{1} = 0;
y{1} = 0;
%data save
...
for k = delay_bound+1:1:T1
    ...
    u{k+1} = u{k} + delta_u{k}; 
%     x{k+1} = x{k} + delta_x{k};
    z{k+1} = Az*z{k} + Bz1*delta_u{k} + Nz*z{k-d}+0*Gp*E*cell2mat(delta_ommega(k));
    x{k+1} = z{k}(1:2) + x{k};
...
for k = T1:1:T2
    ...
    u{k+1} = u{k} + delta_u{k}; 
%     x{k+1} = x{k} + delta_x{k};
    z{k+1} = Az*z{k} + Bz2*delta_u{k} + Nz*z{k-d}+0*Gp*E*cell2mat(delta_ommega(k));
    x{k+1} = z{k}(1:2) + x{k};
    delta_y{k+1} = Cz*z{k+1};
    y{k+1} = y{k} + delta_y{k+1};
    z_rec = [z_rec z{k+1}];
